...
import com.karaka.config.client.KeycloakActionRequestResolver;
...
public class SecurityConfig {
  ...
  /**
   * Public by necessity: someone who cannot sign in has no session, so requiring one would
   * make the forgot-password flow unreachable by the only people who need it. Protection for
   * {@code /api/forgot-password} is rate limiting inside the service, not authentication.
   */
  ...
  private static final String[] PUBLIC_PATHS = {
    ...
    FORGOT_PASSWORD, FORGOT_PASSWORD + "/**", "/api/forgot-password"
  };
  ...
  private OAuth2AuthorizationRequestResolver pkceResolver(
    ...
    // Wrapped so `?kc_action=CONFIGURE_TOTP` on the authorization URL reaches Keycloak, which
    // is how the UI offers "Set up two-factor" as a link instead of building a form. The
    // wrapper allow-lists the actions; see KeycloakActionRequestResolver.
    return new KeycloakActionRequestResolver(resolver);
  }
